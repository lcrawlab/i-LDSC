#!/usr/bin/env python
'''
(c) 2022 Gregory Darnell and Samuel Pattillo Smith
    Adapted from: (c) 2014 Brendan Bulik-Sullivan and Hilary Finucane

MELD is a command line tool for estimating
    1. Marginal epistatic scores
    2. Linear and non-linear variance components of heritability
...
try:
    ...
    raise ImportError('MELD requires pandas version >= 0.17.0')

__version__ = '1.0.1'
MASTHEAD = "*********************************************************************\n"
MASTHEAD += "* Marginal Epistatic LD Score Regression (MELD)\n"
MASTHEAD += "* Version {V}\n".format(V=__version__)
MASTHEAD += "* (C) 2022 Gregory Darnell and Samuel Pattillo Smith\n"
MASTHEAD += "* Brown University\n"
...
def ldscore(args, log):
    ...
    # pull out all but last column of annot_matrix
    #lN = geno_array.ldScoreVarBlocks(block_left, args.chunk_size, annot=annot_matrix)
    lN = geno_array.ldScoreVarBlocks(block_left, args.chunk_size, annot=annot_matrix[:,:-1])
    # save last column of annot_matrix for meld scores
    lM = geno_array.ldScoreVarBlocks(block_left, args.chunk_size, annot=np.expand_dims(annot_matrix[:,-1],1), meld=True)
    lN = np.hstack((lN,lM))
    #n_annot = 2
    #annot_colnames = ["L2","MELD"]
...
parser.add_argument('--out', default='meld', type=str,
    help='Output filename prefix. If --out is not set, MELD will use meld as the '
...
parser.add_argument('--print-snps', default=None, type=str,
    help='This flag tells MELD to only print LD Scores for the SNPs listed '
...
#     help = 'This flag tells MELD the alpha value indicative of MAF dependent architecture '
#     "that should be used in the following format of MELD scores 'Full_MELD_/R2_$alpha/MELD.win_100.@'. ")
...
parser.add_argument('--annot', default=None, type=str,
    ...
    'MELD will automatically append .annot or .annot.gz to the filename prefix. '
...
parser.add_argument('--cts-bin', default=None, type=str,
    help='This flag tells MELD to compute partitioned LD Scores, where the partition '
...
parser.add_argument('--per-allele', default=False, action='store_true',
    help='Setting this flag causes MELD to compute per-allele LD Scores, '
    'i.e., \ell_j := \sum_k p_k(1-p_k)r^2_{jk}, where p_k denotes the MAF '
    'of SNP j. ')
parser.add_argument('--pq-exp', default=None, type=float,
    help='Setting this flag causes MELD to compute LD Scores with the given scale factor, '
...
parser.add_argument('--no-print-annot', default=False, action='store_true',
    help='By defualt, seting --cts-bin or --cts-bin-add causes MELD to print '
    'the resulting annot matrix. Setting --no-print-annot tells MELD not '
...
parser.add_argument('--ref-ld', default=None, type=str,
    help='Use --ref-ld to tell MELD which MELD Scores to use as the predictors in the MELD '
    'Score regression. '
    'MELD will automatically append .l2.ldscore/.l2.ldscore.gz to the filename prefix.')
parser.add_argument('--ref-ld-chr', default=None, type=str,
    ...
    'across 22 chromosomes. MELD will automatically append .l2.ldscore/.l2.ldscore.gz '
    'to the filename prefix. If the filename prefix contains the symbol @, MELD will '
    'replace the @ symbol with chromosome numbers. Otherwise, MELD will append chromosome '
...
parser.add_argument('--overlap-annot', default=False, action='store_true',
    help='This flag informs MELD that the partitioned LD Scores were generates using an '
    'annot matrix with overlapping categories (i.e., not all row sums equal 1), '
    'and prevents MELD from displaying output that is meaningless with overlapping categories.')
...
parser.add_argument('--print-cov', default=False, action='store_true',
    help='For use with --h2/--rg. This flag tells MELD to print the '
    'covaraince matrix of the estimates.')
parser.add_argument('--print-delete-vals', default=False, action='store_true',
    help='If this flag is set, MELD will print the block jackknife delete-values ('
...
parser.add_argument('--invert-anyway', default=False, action='store_true',
    help="Force MELD to attempt to invert ill-conditioned matrices.")
...
parser.add_argument('--not-M-5-50', default=False, action='store_true',
    help='This flag tells MELD to use the .l2.M file instead of the .l2.M_5_50 file.')
parser.add_argument('--return-silly-things', default=False, action='store_true',
    help='Force MELD to return silly genetic correlation estimates.')
...
if __name__ == '__main__':
    ...
    try:
        ...
        # summary statistics
            ...
            # print(args.ref_ld_chr,args.alpha)
            # if args.alpha != '':
            #     parse_args
            ...
            print 'meld.py -h describes options.'
    except Exception:
